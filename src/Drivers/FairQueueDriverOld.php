...
class FairQueueDriverOld extends RedisQueue
{

    /** @var string $workerUuid */
    protected $workerUuid;
     ...
     * @return void
     */
    public function __construct(Factory $redis, $default = 'default', $connection = null, $retryAfter = 60, $blockFor = null)
    {
        $this->workerUuid = Str::uuid();
        dump('======= boot worker fair-queue old ' . $this->workerUuid);
    ...
    {
        $queue = $queue ?? 'default';
        dump('push queue fair queue ' . $queue);
    ...
    {
        dump('pop queue fair queue ' . $queue . ' ' . now());
        $queue = $queue ?? 'default';
